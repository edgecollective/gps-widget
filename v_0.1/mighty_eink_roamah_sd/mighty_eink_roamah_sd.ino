...
void log_data(char locator[]) {
File dataFile = SD.open("datalog.txt", FILE_WRITE);
   if (dataFile) {
    dataFile.println(locator);
    dataFile.close();
...
void setup() {
  ...
  digitalWrite(enablePin, LOW);   // turn the LED on (HIGH is the voltage level)
  delay(1000);
...
void loop() {
 ...
 if (newData)
  ...
  log_data(getLocator(flat,flon, 4));
